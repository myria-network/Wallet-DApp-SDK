...
tokenImage.src = tokenAvatarPng(asset.assetId, asset.symbol, asset.name, networkId);
```

The result is a cached `data:image/png;base64` URL generated by the shared `@myria-network/core` package. Pass the verified Genesis NetworkID as the fourth argument. The artwork is presentation only; applications must still verify the token definition and accepted transactions.
